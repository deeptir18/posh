...
fn run_viz(cmd: &str, name: &str, output_folder: &str, viztype: VizType) {
    match viztype {
        VizType::Shell => match visualize_shell_graph(cmd, name, output_folder) {
            Ok(_) => {}
            Err(e) => {
                println!("Failed to visualize shell graph: {:?}", e);
            }
        },
        VizType::Dash => match visualize_dash_graph(cmd, name, output_folder) {
            Ok(_) => {}
            Err(e) => {
...
fn visualize_shell_graph(command: &str, name: &str, folder: &str) -> Result<()> {
    ...
    // invoke graphviz
    invoke_graph_viz(dot_path_str, graph_path_str)?;
    Ok(())
}

fn visualize_dash_graph(command: &str, name: &str, folder: &str) -> Result<()> {
...
fn invoke_graph_viz(dot_path: &str, graph_path: &str) -> Result<()> {
